  ...
  """
    ...
    def MustSplit(self):
        ...
        if (style.Get('SPLIT_BEFORE_FIRST_ARGUMENT')
                and self.line.first.value != 'def'
                and self.line.first.value in _COMPOUND_STMTS):
            return False
        ...
        if (style.Get('SPLIT_BEFORE_NAMED_ASSIGNS') and not current.is_comment
                and format_token.Subtype.DEFAULT_OR_NAMED_ASSIGN_ARG_LIST in
                current.subtypes):
        ...
        if (previous.OpensScope() and not current.OpensScope()
                and format_token.Subtype.SUBSCRIPT_BRACKET not in
                previous.subtypes):
            ...
            if previous.value == '(':
                ...
                if not pptoken or not pptoken.is_name:
                    ...
                    if (self.column_limit - self.column
                        ) / float(self.column_limit) < 0.3:
        ...
        if current.ClosesScope():
            if (previous.OpensScope() or
                (previous.is_comment and previous.previous_token is not None
                 and previous.previous_token.OpensScope())):
                return max(0, top_of_stack.indent -
                           style.Get('CONTINUATION_INDENT_WIDTH'))
    ...
    def ComplexityPenalty(self):
        """
    Creates a complexity penalty for the current token.
    """
        # print "# ", "moving to next token, ",self.next_token.previous_token.value, self.next_token.value
        ...
        if token_value in {"for", "in", "if", "else", "not"}:
            return high

        if token_value in {".", "()", "in"}:
            return med
